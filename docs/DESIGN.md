...
> - **CSV imports (after M3):** `domain/csvRead.ts` holds the shared reading (delimiter, quotes, BOM, number cells, date order with seconds). `domain/xlsx.ts` reads an .xlsx's first sheet (zip directory + sheet XML, shared strings, date-styled serials) and `platform/spreadsheet.ts` inflates parts with `DecompressionStream`, so the importers get CSV text either way. `domain/scaleCsv.ts` and `domain/cronometerCsv.ts` match headers on top of it. `services/nutrition/scaleImport.ts` and `cronometerImport.ts` preview per day, then write in one transaction; imported values replace typed ones (`replaceExisting` defaults to true for the scale).
