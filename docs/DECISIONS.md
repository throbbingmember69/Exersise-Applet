...
| Scale and food imports | File imports for now: the smart scale's export (Arboleaf gives an Excel .xlsx file; CSV also accepted) and Cronometer's Daily Nutrition or Servings export. Scale: one entry per day, each value from the day's earliest reading that has it (the scale logs a weight-only reading seconds before the full one). **Imported data wins:** a day in the file replaces what was typed for it (the scale import can opt to keep typed days). Nutrition imports never touch steps. |
