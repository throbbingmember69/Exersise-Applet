...
import { mainLiftSlide, type StrengthSeries } from './strength'

const AS_OF = parseLocalDate('2026-10-30')
const S = DEFAULT_SETTINGS

/** A point `daysAgo` before AS_OF. */
...
describe('mainLiftSlide', () => {
  ...
  it('includes both ends of each window and nothing between them', () => {
    const edges = mainLiftSlide(
      [{ key: 'a', points: [pt(27, 100), pt(6, 90), pt(7, 50), pt(20, 500), pt(28, 500)] }],
      AS_OF,
      S,
    )
    expect(edges.perLift[0]).toMatchObject({ earlierBestLb: 100, recentBestLb: 90 })
  })
  ...
  it('uses the configured threshold and windows', () => {
    ...
    const r = mainLiftSlide(
      [{ key: 'a', points: [pt(16, 200), pt(13, 300), pt(2, 196)] }],
      AS_OF,
      s,
    )
    expect(r.windows).toEqual({
    ...
    expect(r.perLift[0]).toMatchObject({ earlierBestLb: 200, changePct: -2 })
    expect(r.triggered).toBe(false)
    expect(
