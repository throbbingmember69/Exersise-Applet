...
const START: TrackStart = deepFreeze({ startLoadLb: 220, calibrate: false })

const state = (patch: Partial<TrackState>): TrackState =>
  deepFreeze({ ...initialTrackState(), ...patch })

/** repTargets on a frozen copy of `lastReps`. */
const targets = (
  branch: Branch,
  ...
  repMin: number,
  repMax: number,
) => repTargets(branch, deepFreeze(lastReps), sets, repMin, repMax)

/** deloadPrescription on frozen inputs. */
const deload = (
  p: NextPrescription,
  regime: Regime,
  stepLb: number,
  loadType: LoadType,
  settings: Settings,
) => deloadPrescription(deepFreeze(p), deepFreeze(regime), stepLb, loadType, deepFreeze(settings))

describe('repTargets', () => {
  it('same_plus_rep: last + 1 per set, capped at repMax; repMin where there is no last', () => {
    expect(targets('same_plus_rep', [8, 10, 9, null], 5, 6, 10)).toEqual([9, 10, 10, 6, 6])
  })

  it('same_after_miss: last reps clamped into the range, no +1', () => {
    expect(targets('same_after_miss', [8, 5, 0, 12], 4, 6, 10)).toEqual([8, 6, 6, 10])
    expect(targets('same_after_miss', [], 2, 6, 10)).toEqual([6, 6])
  })

  it('keeps +1 targets inside a rep range that has since moved up', () => {
    expect(targets('same_plus_rep', [6, 7], 2, 10, 15)).toEqual([10, 10])
  })

  it('every other branch targets repMin', () => {
    for (const branch of ['start', 'calibration', 'calibrated', 'step', 'drop'] as const) {
      expect(targets(branch, [9, 9, 9], 3, 8, 12)).toEqual([8, 8, 8])
    }
  })

  it('returns one target per prescribed set', () => {
    expect(targets('same_plus_rep', [8, 8, 8, 8], 2, 6, 10)).toEqual([9, 9])
    expect(targets('step', [], 0, 6, 10)).toEqual([])
  })
})

describe('nextPrescription', () => {
  const next = (s: TrackState, start = START, stepLb = 10, loadType: LoadType = 'machine') =>
    nextPrescription(deepFreeze(s), deepFreeze(start), SQUAT, stepLb, loadType, S)
  ...
  it('uses the regime set count even when it changed since the last session', () => {
    ...
    const threeSets = deepFreeze({ ...SQUAT, sets: 3 })
    expect(nextPrescription(s, START, threeSets, 10, 'machine', S)).toMatchObject({
      sets: 3,
      repTargets: [9, 9, 6],
    })
  })
...
describe('deloadPrescription', () => {
  const base: NextPrescription = deepFreeze({
    loadLb: 220,
    repTargets: [9, 9, 8, 7],
    sets: 4,
  ...
  it('halves the sets (rounding up) and cuts the load 10% in whole steps', () => {
    expect(deload(base, SQUAT, 10, 'machine', S)).toEqual({
      ...base,
      loadLb: 200,
      sets: 2,
      repTargets: [9, 9],
    ...
    expect(deload(base, threeSets, 10, 'machine', S).sets).toBe(2)
    const oneSet = { ...SQUAT, sets: 1 }
    expect(deload(base, oneSet, 10, 'machine', S).sets).toBe(1)
  })

  it('keeps a blank load blank and existing notices', () => {
    const blank = { ...base, loadLb: null, notices: [{ code: 'calibration_needed' as const }] }
    expect(deload(blank, SQUAT, 5, 'machine', S)).toMatchObject({
      loadLb: null,
  ...
  it('cuts bodyweight-plus added load below zero', () => {
    ...
    expect(deload(chin, SQUAT, 5, 'bodyweight_plus', S).loadLb).toBe(-5)
  })

  it('uses the deload settings', () => {
    const s = resolveSettings({ deloadSetFraction: 0.75, deloadLoadCutPct: 20 })
    expect(deload(base, SQUAT, 10, 'machine', s)).toMatchObject({
      sets: 3,
      loadLb: 180,
      repTargets: [9, 9, 8],
    })
  ...
  it('is not thrown off by binary error in the set fraction', () => {
    ...
    expect(deload(base, { ...SQUAT, sets: 10 }, 10, 'machine', s).sets).toBe(3)
  })

  it('pads rep targets with repMin when the prescription has fewer than the deload sets', () => {
    const short = { ...base, repTargets: [9] }
    expect(deload(short, SQUAT, 10, 'machine', S).repTargets).toEqual([9, 6])
  })
})
