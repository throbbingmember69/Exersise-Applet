...
// to zero or below (assisted); every other load is clamped at 0. The one-step minimum applies at
// every pct, including 0 (DESIGN.md (c) and the M1 brief give no exception for a 0% cut).
...
export function dropLoad(
  ...
  if (!(pct >= 0) || !Number.isFinite(pct)) throw new RangeError(`pct must be >= 0, got ${pct}`)
